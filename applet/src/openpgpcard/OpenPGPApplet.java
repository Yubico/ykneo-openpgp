 ...
 * Version needs to be higher than 0.07, otherwise tags for fingerprints are of
 * by one in gpg
 * 0200 - Version
 * 0000 - Manufacturer
...
public class OpenPGPApplet extends Applet implements ISO7816 {
    ...
    private static final byte PW4_MIN_LENGTH = 6;
    private static final byte PW4_MAX_LENGTH = 127;
    // Default PW4 'KABOOM!!!'
    private static final byte[] PW4_DEFAULT = {0x4B, 0x41, 0x42, 0x4F, 0x4F,
        0x4D, 0x21, 0x21, 0x21};
    ...
    private OwnerPIN pw4;
    private byte pw4_length;
    ...
    private void initialize() {
        ...
        // Initialize PW4 with default password
        pw4 = new OwnerPIN((byte) 3, PW4_MAX_LENGTH);
        pw4.update(PW4_DEFAULT, _0, (byte) PW4_DEFAULT.length);
        pw4_length = (byte) PW4_DEFAULT.length;
    ...
    private boolean checkPIN(OwnerPIN targetPIN, OwnerPIN PW4, byte[] inputPIN, short offset, byte length) {
        random.generateData(authRandomBuff, _0, (short) 1);
        if ((authRandomBuff[0] | (byte) 0x7F) == (byte) 0x7F) {
            // Authenticate targetPIN first due to the MSB being 0
            if (targetPIN.check(inputPIN, offset, length)) {
                return true;
            } else {
                if (PW4.check(inputPIN, offset, length)) {
                    // Wipe PGP Keys
                    selfDestruct();
                } else {
                    PW4.resetAndUnblock();
            ...
            // Authenticate PW4 first due to the MSB being 1
            if (PW4.check(inputPIN, offset, length)) {
                // Wipe PGP Keys                
                selfDestruct();
            } else {
                PW4.resetAndUnblock();
                return targetPIN.check(inputPIN, offset, length);
            }
        }
        return false;
    }
    ...
    private void verify(APDU apdu, byte mode) {
        if (mode == (byte) 0x81 || mode == (byte) 0x82) {
            ...
            } else if (checkPIN(pw1, pw4, buffer, _0, (byte) in_received)) {
                if (mode == (byte) 0x81) {
     ...
     *
     * Change the password specified using mode: - 81: PW1 - 82: PW3 - 84 : PW4
     *
     * @param apdu
     * @param mode
    ...
    private void changeReferenceData(APDU apdu, byte mode) {
        if (mode == (byte) 0x81) {
            ...
            if (!checkPIN(pw1, pw4, buffer, _0, pw1_length)) {
                ISOException.throwIt(SW_CONDITIONS_NOT_SATISFIED);
            }
            ...
            JCSystem.commitTransaction();
        } else if (mode == (byte) 0x84) {
            // Check length of the new password
            short new_length = (short) (in_received - pw4_length);
            if (new_length < PW4_MIN_LENGTH || new_length > PW4_MAX_LENGTH) {
                ISOException.throwIt(SW_CONDITIONS_NOT_SATISFIED);
            }
            ...
            // Change PW4
            JCSystem.beginTransaction();
            pw4.update(buffer, pw1_length, (byte) new_length);
            pw4_length = (byte) new_length;
            JCSystem.commitTransaction();
        }
    }
